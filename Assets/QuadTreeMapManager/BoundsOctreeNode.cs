...
public class BoundsOctreeNode<T>
{
    ...
    */
    ...
    public void TriggerMove(Camera cam)
    {
        if (children != null)
        {
            for (int i = 0, len = children.Length; i < len; ++i)
            {
                if (children[i].bounds.CheckBoundIsInCamera(cam))
                {
                    children[i].TriggerMove(cam);
                }
                else
                {
                    children[i].TriggerOutside();
                }
            }
        }
        ...
        {
            ObjManager.Instance.LoadAsync(objects[i].Obj as ObjData);
        }
    }

    private void TriggerOutside()
    {
        if (children != null)
        {
            for (int i = 0, len = children.Length; i < len; ++i)
        ...
        for (int i = 0, len = objects.Count; i < len; ++i)
        {
            if (objects[i].Obj is ObjData objData)
            {
                ObjManager.Instance.Unload(objData.uid);
            }
        }
    }
}